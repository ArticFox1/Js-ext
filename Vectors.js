(function (Scratch) {
  ...
  const div = [div2, div3, div4]

  //Extension Functions
  function addition(a, b) {
    return add[Math.max(a.length, b.length) - 2](a, b)
  }
  function subtraction(a, b) {
    return sub[Math.max(a.length, b.length) - 2](a, b)
  }
  function divide(a, b) {
    return div[Math.max(a.length, b.length) - 2](a, b)
  }
  function length(v) {
    if (!Number.isFinite(v.x)) return v
    return len[getLength(v) - 2](v)
  }
